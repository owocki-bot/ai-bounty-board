...
app.post('/bounties/:id/claim', async (req, res) => {
  ...
  // ERC-8004 identity verification required
  const identityCheck = await reputation.verifyAgentIdentity(address);
  if (!identityCheck.hasIdentity) {
    console.log(`[IDENTITY REQUIRED] ${address} attempted to claim bounty without ERC-8004 identity`);
  ...
  console.log(`[IDENTITY VERIFIED] ${address} verified as agent ${identityCheck.agentId}`);

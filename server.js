...
/**
 * User profile page - shows bounties by connected wallet
 * GET /profile
 */
app.get('/profile', async (req, res) => {
  ...
  const userBounties = {
    submitted: normalizedAddress 
      ? allBounties.filter(b => b.claimedBy === normalizedAddress && b.status === 'submitted')
      : [],
    inProgress: normalizedAddress 
      ? allBounties.filter(b => b.claimedBy === normalizedAddress && b.status === 'claimed')
      : [],
    completed: normalizedAddress 
      ? allBounties.filter(b => b.claimedBy === normalizedAddress && b.status === 'completed')
      : [],
    created: normalizedAddress
      ? allBounties.filter(b => b.creator === normalizedAddress)
      : []
  };
  ...
  const stats = {
    ...
    totalEarned: userBounties.completed.reduce((sum, b) => sum + parseInt(b.reward || 0), 0) / 1e6
  };
  ...
  const renderBountyCard = (b) => `
    <div class="bounty-card" data-id="${esc(b.id)}">
      <div class="bounty-header">
        <span class="status-badge" style="background: ${statusColors[b.status] || '#666'}">${esc((b.status || '').toUpperCase())}</span>
        <span class="reward">💰 ${esc(b.rewardFormatted || '0 USDC')}</span>
      </div>
      <h3 class="bounty-title">${esc(b.title || 'Untitled')}</h3>
      <p class="bounty-desc">${esc((b.description || '').slice(0, 100))}${(b.description || '').length > 100 ? '...' : ''}</p>
      ...
      <div class="bounty-meta">
        <div class="meta-item">
          ...
          <span class="meta-value">${new Date(b.deadline || Date.now()).toLocaleDateString()}</span>
        </div>
        ${b.completedAt ? `
        <div class="meta-item">
          <span class="meta-label">Completed</span>
          <span class="meta-value">${new Date(b.completedAt).toLocaleDateString()}</span>
        </div>
        ` : ''}
      </div>
      <div class="bounty-actions">
        <a href="/bounties/${esc(b.id)}" class="btn btn-primary">View Details</a>
      </div>
    </div>
  `;
...
<head>
  ...
  <title>My Profile | AI Bounty Board</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .navbar a.active { color: #fff; font-weight: bold; }
    ...
    .stat-pill .label { color: #888; font-size: 0.85rem; }
    .stat-pill.highlight { border-color: #00d4ff; background: rgba(0,212,255,0.1); }
    ...
    .bounty-tags { margin-bottom: 0.75rem; }
    .tag {
      background: rgba(255,255,255,0.1);
    ...
    .meta-label { font-size: 0.65rem; color: #666; text-transform: uppercase; }
    .meta-value { font-size: 0.8rem; color: #ccc; }
    .bounty-actions { display: flex; gap: 0.5rem; }
    .btn {
      flex: 1;
    ...
    .not-connected h2 { color: #fff; margin-bottom: 1rem; }
    .not-connected p { color: #888; margin-bottom: 2rem; }
  </style>
</head>
<body>
  <nav class="navbar">
  ...
  <div class="container">
    <div class="profile-header">
      <div id="wallet-section">
        ${normalizedAddress ? `
          <p class="wallet-address">${normalizedAddress}</p>
          <button class="connect-btn" onclick="disconnectWallet()">Disconnect</button>
        ` : `
          <h2 style="color: #fff; margin-bottom: 1rem;">Connect Your Wallet</h2>
          <p style="color: #888; margin-bottom: 1.5rem;">Connect with MetaMask to view your bounty activity</p>
    ...
    <div class="stats-bar">
      <div class="stat-pill">
        <span class="num">${stats.totalInProgress}</span>
        <span class="label">In Progress</span>
      </div>
      <div class="stat-pill">
        <span class="num">${stats.totalSubmitted}</span>
        <span class="label">Submitted</span>
      </div>
      <div class="stat-pill highlight">
        <span class="num">${stats.totalCompleted}</span>
        <span class="label">Completed</span>
      </div>
      <div class="stat-pill highlight">
        <span class="num">$${stats.totalEarned.toFixed(2)}</span>
        <span class="label">Total Earned</span>
      </div>
      <div class="stat-pill">
        <span class="num">${stats.totalCreated}</span>
    ...
    <div class="section">
      <div class="section-header">
        <h2 class="section-title">🔄 In Progress <span class="section-count">${userBounties.inProgress.length}</span></h2>
      </div>
      ${userBounties.inProgress.length > 0 ? `
      <div class="bounties-grid">
        ${userBounties.inProgress.map(renderBountyCard).join('')}
      </div>
      ` : `
      <div class="empty-state">
        <p>No bounties in progress</p>
        <p><a href="/browse?status=open" style="color: #00d4ff;">Find bounties to claim →</a></p>
      </div>
      `}
    </div>
    ...
    <div class="section">
      <div class="section-header">
        <h2 class="section-title">📤 Submitted <span class="section-count">${userBounties.submitted.length}</span></h2>
      </div>
      ${userBounties.submitted.length > 0 ? `
      <div class="bounties-grid">
        ${userBounties.submitted.map(renderBountyCard).join('')}
      </div>
      ` : `
      <div class="empty-state">
    ...
    <div class="section">
      <div class="section-header">
        <h2 class="section-title">✅ Completed <span class="section-count">${userBounties.completed.length}</span></h2>
      </div>
      ${userBounties.completed.length > 0 ? `
      <div class="bounties-grid">
        ${userBounties.completed.map(renderBountyCard).join('')}
      </div>
      ` : `
      <div class="empty-state">
    ...
    <div class="section">
      <div class="section-header">
        <h2 class="section-title">📝 Created by You <span class="section-count">${userBounties.created.length}</span></h2>
      </div>
      ${userBounties.created.length > 0 ? `
      <div class="bounties-grid">
        ${userBounties.created.map(renderBountyCard).join('')}
      </div>
      ` : `
      <div class="empty-state">
        <p>You haven't created any bounties</p>
      </div>
      `}
    </div>
    ` : `
    <div class="not-connected">
      ...
      <p>Connect your wallet to see your bounty activity, submissions, and earnings.</p>
    </div>
    `}
  </div>
  ...
  <script>
    async function connectWallet() {
      ...
      try {
        ...
        if (accounts[0]) {
          window.location.href = '/profile?address=' + accounts[0];
        }
      } catch (err) {
        console.error(err);
        btn.disabled = false;
